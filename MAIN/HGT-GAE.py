...
def KG_data_prepare():
    ...
    node_features = {}
    for ntype in KG.ntypes:
        emb = nn.Parameter(torch.Tensor(KG.number_of_nodes(ntype), 400), requires_grad=False)  # .to(device)
        nn.init.xavier_uniform_(emb)
        KG.nodes[ntype].data['feature'] = emb
        node_features[ntype] = emb

    KG = KG.to(device)
    model_KG = HGT_PF(KG, 400, 200, 16, n_layers=2, n_heads=4, use_norm=True).to(device)
...
def SN_data_prepare():
    ...
    print(SN)
    # 生成每个节点的特征向量
    emb = nn.Parameter(torch.Tensor(len(SN.nodes()), 400), requires_grad=False)#.to(device)
    nn.init.xavier_uniform_(emb)
    SN.ndata['feat'] = emb
...
if __name__ == '__main__':
    ...
    model = Model_HGT_GAE(model_KG, model_SN)
    ...
    for epoch in range(args.epochs):
        ...
        pos_score, neg_score, logits, features = model(KG, negative_graph, triplet, SN, feats)
        ...
        loss = loss_KG + 0.5*loss_SN + loss_align
