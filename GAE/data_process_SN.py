...
def mask_test_edges_dgl(graph, adj, etype):
...
def get_score(graph, edges_pos, edges_neg, triplet):
    ...
    for src, dst in edges_pos:
        ...
        preds.append(torch.sigmoid(res))
    ...
    for src, dst in edges_neg:
        ...
        dst_v = graph.nodes[dst_type].data['h'][dst]
        res = torch.mul(src_v, dst_v).sum()
        preds_neg.append(torch.sigmoid(res))
    preds_all = np.hstack([preds, preds_neg])
    labels_all = np.hstack([np.ones(len(preds)), np.zeros(len(preds_neg))])
    roc_score = roc_auc_score(labels_all, preds_all)
    # 这里ap_score会报错，’Process finished with exit code 139‘
