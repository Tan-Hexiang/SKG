...
if __name__ == '__main__':
    ...
    G = read_data()
    print(G)
    triplet = ('paper', 'citing', 'paper')

    # 确定训练集、验证集和测试集
    # 这里的邻接矩阵作用只是提供idx的范围
    adj_orig = G.adjacency_matrix(etype=triplet[1]).to_dense()
    # print(adj_orig.shape)
    # exit()
    # build test set with 10% positive links
    train_edge_idx, val_edges, val_edges_false, test_edges, test_edges_false = mask_test_edges_dgl(G, adj_orig, triplet[1])
    ...
    #     ('paper', 'citing', 'paper'): train_edge_idx#list(range(G.number_of_edges('citing'))),
    ...
    # }, preserve_nodes=True)
    src, dst = G.edges(etype=triplet[1])
    # print(type(src))
    # print(src, dst)
    # exit()
    train_graph = dgl.heterograph({('paper', 'citing', 'paper'): (src[train_edge_idx], dst[train_edge_idx])})
    print(train_graph)
    G = train_graph
    ...
    for epoch in range(1000):
        # 这里是在整张图上做训练
        model.train()
        negative_graph = construct_negative_graph(G, 1, triplet)
        # node_features = {}
        # for ntype in G.ntypes:
        #     node_features[ntype] = G.nodes[ntype].data['feature']
        pos_score, neg_score = model(G, negative_graph, triplet)
        ...
        if train_step % 10 == 0:
            val_roc, val_ap = get_score(G, val_edges, val_edges_false, triplet)
